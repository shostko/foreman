...
val Status.isWorkingRefresh: Boolean
    get() = if (this is CorePagingStatus) isWorkingRefresh else isWorking
val Status.isWorkingAppend: Boolean
    get() = if (this is CorePagingStatus) isWorkingAppend else false
val Status.isWorkingPrepend: Boolean
    get() = if (this is CorePagingStatus) isWorkingPrepend else false
val Status.throwableRefresh: Throwable?
    get() = if (this is CorePagingStatus) throwableRefresh else null
val Status.throwableAppend: Throwable?
    get() = if (this is CorePagingStatus) throwableAppend else null
val Status.throwablePrepend: Throwable?
    get() = if (this is CorePagingStatus) throwablePrepend else null

fun Status.extractRefresh(): Status = when {
    isInitial -> this
    this is CorePagingStatus -> Status.create(isWorkingRefresh, throwableRefresh)
...
fun Status.extractAppend(): Status = when {
    isInitial -> this
    this is CorePagingStatus -> Status.create(isWorkingAppend, throwableAppend)
...
fun Status.extractPrepend(): Status = when {
    isInitial -> this
    this is CorePagingStatus -> Status.create(isWorkingPrepend, throwablePrepend)
    this is RefreshingStatusWrapper -> Status.Initial
    else -> throw RuntimeException("Can't extract prepend status from not PagingStatus")
}

fun Status.asRefreshingStatusWrapper(): Status = if (isInitial || this is CorePagingStatus) this else RefreshingStatusWrapper(this)
